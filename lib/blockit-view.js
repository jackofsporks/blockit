...
// Temporary reprieve
import getChildrenNode from './control/getChildrenNode'
...
export default class BlockitView {
  ...
  getContents = (value) => {
    this.element.output.innerHTML = getChildrenNode(value)
  }
  ...
  subscribe (editor) {
    ...
    input.addEventListener("change", this.getContents)
    const inputDisp = new Disposable(function(){input.removeEventListener("change",this.getContents)})
    subs.add(inputDisp)

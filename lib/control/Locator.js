...
export default class Renderer {
  ...
  deleteOne(e, beforeCursor, afterCursor, fileScope, previousBlock){
    ...
    } else {console.log("else -->", e);data = {color: "purple", block: {}}}
    this.deleting = false
    return data
  }
  ...
  locate = async (e, beforeCursor, afterCursor, fileScope, previousBlock) => {
    ...
    if (e.eventType === "cursor") {
      action = "move"
      if (this.deleting) {return {color: "paleblue",block:previousBlock}} // If we're deleting then pretend this never happened. almost....

    ...
    rdr.finders = { // boundries????!?!? that are finding rules. rule finders by boundry characters
      "html": {
        ...
        "<": function(beforeTrigger, afterTrigger, afterCursor){
          ...
          for (let ii = lastIndex; ii >= 0; ii--) {
            ...
            if (character === "<" && found.type !== "attribute") {
              ...
              // TODO assign this to found.attribute unless/until we find it's a value
              found.nameStartIndex = ii+1
          ...
          if (found.type === "tag") found.name = tag
            ...
            block.startIndex = beforeTrigger.length + attributeEndInclusive - found.attribute.length // - 1 more?!?!?
        ...
        "_value": function(beforeTrigger, afterTrigger, afterCursor, found){
          ...
          // QUESTION allow line breaks or tabs between values?!?!?
          const fromStart = declaration.substring(found.nameStartIndex)
          found.name = /^([^" ]*)/.exec(fromStart)[1]
          block.value = found.name
          ...
          const myLength = me.name.length
          const myBlockStart = 5

          const before = declarationStart.substring(0,nameStartIndex)
          const all = declarationStart + declarationEnd
          const after = all.substring(nameStartIndex + myLength)

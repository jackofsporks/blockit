...
export default class Locator {
  ...
  getBlock (e, beforeCursor, afterCursor, fileScope, previousBlock) {
    ...
    const data = build(block, beforeFound, afterFound, afterCursor)
    return data
  }
  ...
  delete(e, beforeCursor, afterCursor, fileScope, previousBlock){
    this.shouldFollowup = this.endChangingBuffer
    this.followUp = this.getBlock
    // const locations = previousBlock.whereDelete(e, beforeCursor, afterCursor) // selection?? multiple cursors??
    const data = [{location:{start: 0, end: 5}, deleted:""},{location:{start: 8, end: 10}, deleted:""},{location:{start: 13, end: 18}, deleted:""}]
    const hiker = this
    // Return deletion first before moving!!!!
    setTimeout(function(){hiker.bufferChanger.deleteIndexRange(data)},0)
    // wait for editor to update
    // #move# with new cursor position
  ...
  endChangingBuffer(){
    const changingSelf = this.bufferChanger.changingSelf
    this.bufferChanger.endChange() // in moving instead??
    return changingSelf
  }

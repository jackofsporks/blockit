...
export default class Renderer {
  ...
  async deleteBlock(e, previousBlock){
    // remove text (TODO and any extra whitespace)
    console.log(previousBlock.startIndex,
previousBlock.endIndex);
    let fridge = await this.bufferChanger.deleteIndexRange({start: previousBlock.startIndex || 0, end: previousBlock.endIndex || 0})
    console.log("fridge -->", fridge);
    // wait for editor to update
    // #move# with new cursor position
    return {color: "green", block: {}}
  }

  async move(e, beforeCursor, afterCursor, fileScope, previousBlock){
  ...
  async deleteMany(e, beforeCursor, afterCursor, fileScope, previousBlock){return {color: "black", block: {}}}
  async deleteOne(e, beforeCursor, afterCursor, fileScope, previousBlock){
    ...
    console.log("one a -->", e.oldText, previousBlock.deleters, isDeleter, e);
    if (isDeleter){
      console.log("two -->", e.oldText, previousBlock.deleters, isDeleter);
      return await this.deleteBlock(e, previousBlock)
    } else {console.log("else");return {color: "purple", block: {}}}
  }
  async insertMany(e, beforeCursor, afterCursor, fileScope, previousBlock){return {color: "black", block: {}}}
  async insertOne(e, beforeCursor, afterCursor, fileScope, previousBlock){return await this.move(e, beforeCursor, afterCursor, fileScope, previousBlock)}
  async special(e, beforeCursor, afterCursor, fileScope, previousBlock){console.log("special!!!");return {color: "black", block: {}}}
  ...
  async locate(e, beforeCursor, afterCursor, fileScope, previousBlock){
    ...
    const data = await this[action](e, beforeCursor, afterCursor, fileScope, previousBlock)
    return data
  }
}

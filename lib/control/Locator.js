...
export default class Renderer {
  ...
  locate(e, beforeCursor, afterCursor, cursorPosition, fileScope){
    ...
    // #delete#
    // if block-deleter/block-breaker then delete whole block using... text postition converted to buffer position?!?!?! + #move#
    // else permit and #id#
    // #insert 1#
    // go to #id#
    // #id#
    ...
    // QUESTION TODO listen for editor changes too/instead
    let moveRegexp = /arrow|space|tab|enter/i
    let characterCodeRegexp = /key|quote/i
    ...
    switch (e.type) {
      case "click":
        action = "move"
      case "keydown":
        if (moveRegexp.test(e.key)) action = "move"
        if (e.key === "Backspace") action = "delete 1"
        // insert 1 key
        if (characterCodeRegexp.test(e.code)) {
          // except not undo!!!
          if (e.key === "z" && e.metaKey) action = "delete many" //!??!!? other action?!?!? "special" atm
          // any other action that doesn't insert characters
          else if (e.metaKey) action = "ignore" //!??!!? other action?!?!? "special" atm
          else action = "insert 1"
        }
      default: console.log("action defaulted maybe thru cascade -->", action, e);
    }

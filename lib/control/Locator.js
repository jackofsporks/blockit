...
export default class Locator {
  ...
  constructor (serializedState, bufferChanger, manager) {
    ...
    lctr.languages = { // boundries????!?!? that are finding rules. rule finders by boundry characters
      html: new HTMLBlocks()
    } // languages
    lctr.handler = null
  }
  ...
  getBlock (e, beforeCursor, afterCursor, fileScope, previousBlock) {
    ...
    const build = this.languages[fileScope].getBuild(trigger) // the function that will build the block
    // If no matches were found (might happen at the very start) <-- y?
    if (!build) return null
    const data = build(new Block(), beforeFound, afterFound, afterCursor)
    return data
  }
  ...
  /*Basically undo the action but it includes cursor movement as undoable*/
  cancelAction (e) {
    return "fridge"
  }
  ...
  delete(e, beforeCursor, afterCursor, fileScope, previousBlock){
    ...
    this.followUp = this.getBlock
    // TODO some deletions are actually editing other locations like with tag names id:86 gh:98 ic:gh
    // const locations = this.handler.handle("delete", e, beforeCursor, afterCursor, previousBlock) // selection?? multiple cursors??
    // if (locations.action === "edit")
  ...
  locate = async (e, beforeCursor, afterCursor, fileScope, previousBlock) => {
    this.handler = this.languages[fileScope || "html"]

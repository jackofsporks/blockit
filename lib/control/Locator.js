    ...
    rdr.finders = { // boundries????!?!? that are finding rules. rule finders by boundry characters
      "html": {
        ...
        "<": function(beforeTrigger, afterTrigger, afterCursor){
          ...
          const declarationEnd = /^([^>][\s\S]*>)[\s\S]*/.exec(afterCursor)[1]
          ...
          if (found.type === "tag") found.value = tag
            ...
            rdr.finders.html._value(beforeTrigger, afterTrigger, afterCursor, found)

            // // QUESTION allow line breaks or tabs between values?!?!?
            // const names = /^([^" ]\S*)/.exec(fromStart)
            // if (names !== null) found.name = ""
            // else found.name = names[1]
            ...
            // console.log(parts.length, block.attribute, parts);
            // // Index of start of the attribute in whole thang
            // block.startIndex = beforeTrigger.length + attributeEndInclusive - block.attribute.length // - 1 more?!?!?
            // let relativeIndex = afterCursor.exec(/^[^"]*(")?/).index
            // block.endIndex = beforeTrigger.length + afterTrigger.length + relativeIndex
            // // all: <some-tag-or-partial-tag maybeSom|eAttributeOrAttributePartial="n" class="a-class too" > and other stuff
          }
        ...
        "_value": function(beforeTrigger, afterTrigger, afterCursor, found){
          // all: <some-tag-or-partial-tag maybeSom|eAttributeOrAttributePartial="n" class="a-class too" >
          // build the habitat of the value and its block
          // Habitat
          // Character to left of cursor (maybe to watch for deleting but what about backspace?!?)
          // Full word
          // Parts of word?!!? for fuzzy matching
          // Values and types of nested parents and siblings
          // buffer positions or index of start and end
          // illegal patterns
          // undeletable characters/patterns?!!?
          // uneditable characters/patterns?!?!

          // beforeTrigger, afterTrigger, afterCursor, found
          // found {type, illegalPattern, illegalActions, tag, symbol, nameStartIndex}

          // QUESTION block contains its habitat?!!?
          const block = {type:"attribute"}
          block.now = found
          ...
          // Index of start of the attribute in whole thang
          block.startIndex = beforeTrigger.length + found.attributeEndInclusive - block.attribute.length // - 1 more?!?!?
          let relativeIndex = /^[^"]*(")?/.exec(afterCursor).index
          block.endIndex = beforeTrigger.length + afterTrigger.length + relativeIndex
          console.log(beforeTrigger.length, afterTrigger.length, relativeIndex);
          console.log(block.startIndex, block.endIndex);
          // all: <some-tag-or-partial-tag maybeSom|eAttributeOrAttributePartial="n" class="a-class too" > and other stuff

          block.text = (beforeTrigger + declaration).substring(block.startIndex, block.endIndex + 1)
          console.log("hoooole block ::", block.text);

          // all values in block
          // TODO find duplicate values in the block and don't do that!!!! (compare block.values to each other when moving)
          // block.values = /="([^"][\s\S]*)"$/.exec(block.text)[1].split(" ")
          ...
          const attributesAfter = after.match(attributeBlocksText)
          let attributeBlocksText = []
          if (attributesBefore) attributeBlocksText = attributeBlocksText.concat(attributesBefore)
          if (attributesAfter) attributeBlocksText = attributeBlocksText.concat(attributesAfter)

          for (let ii = 0; ii < attributeBlocksText.length; ii++) {
            let attributeText = attributeBlocksText[ii]
            console.log(attributeText);
            let sib = {}
            // sib.value = attributeText.match(/\s([^=]*)+=/)[1]
            // sib.type = "attribute"
            // sib.values = attributeText.match(/"([^"]+)"/)[1].split(" ")
            // block.siblings.push(sib)
          }

          block.parent = {
            type: "tag",
            value: block.tag,
            children: [block].concat(block.siblings)
          }
          return block
        },
        ">": function(beforeTrigger, afterTrigger, afterCursor){

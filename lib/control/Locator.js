...
export default class Locator {
  ...
  constructor(serializedState, bufferChanger, manager) {
    ...
    lctr.finders = { // boundries????!?!? that are finding rules. rule finders by boundry characters
      "html": {
        ...
        "<": function(beforeTrigger, afterTrigger, afterCursor){
          ...
          let boundries = {
            // "<": function(){
            //   // tag
            // }, //QUESTION included in afterTrigger?!?! id:46 gh:52 ic:gh
            " ": function(){
              // attribute or value
            },
            // "(": function(){},
          ...
          if (found.type === "tag") found.value = tag
            ...
            block = lctr.finders.html._value(beforeTrigger, afterTrigger, afterCursor, found)
          }
        ...
        "_value": function(beforeTrigger, afterTrigger, afterCursor, found){
          ...
          if (attributesBefore) siblingsBefore = siblingsBefore.concat(lctr.finders.html._attributeSiblings(attributesBefore, block.tag))
          if (attributesAfter) siblingsAfter = siblingsAfter.concat(lctr.finders.html._attributeSiblings(attributesAfter, block.tag))
        ...
        "_attributeSiblings": function(allAttributes, tag){
          ...
          // TODO Special highlight when token is a duplicate of siblings but allow to continue editing
          return siblings
        }, //_attributeSiblings
        ">": function(beforeTrigger, afterTrigger, afterCursor){
          // Looking for blocks to be inside an HTML element
          // We got all the text before the closing angle bracket, ">"
          const data = lctr.findTag(beforeTrigger)
  ...
  // findFromCharacterBefore(text, stopper){
  //   let textAfter = null
  //   for (let ii = text.length - 1; ii >= 0; ii--) {
  //     if (text[ii] === stopper) {
  //       textAfter = text.substring(ii)
  //       break
  //     }
  //   }
  //   return index
